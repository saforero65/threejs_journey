...
async function buildProject(project) {
  ...
  try {
    ...
    if (!(await fs.pathExists(projectPath))) {
      console.log(`⚠️  Project ${project.folder} not found, skipping...`);
      return false;
    }
    ...
    if (!(await fs.pathExists(packageJsonPath))) {
      console.log(
        `⚠️  No package.json found in ${project.folder}, skipping...`
      );
      return false;
    }
    ...
    // Copiar el dist al directorio público
    ...
    if (await fs.pathExists(distPath)) {
      ...
      console.log(`❌ Build failed for ${project.name} - no dist folder found`);
      return false;
    }
  } catch (error) {
    console.error(`❌ Error building ${project.name}:`, error.message);
    return false;
  }
}
...
async function generateProjectsData() {
  ...
  console.log(
    `📊 Generated projects data with ${builtProjects.length} projects`
  );
...
async function generateThumbnails() {
  ...
  // Generar thumbnails por defecto (se pueden reemplazar con capturas reales)
  for (const project of projectsConfig) {
...
<svg width="400" height="300" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="50%" y="40%" text-anchor="middle" fill="white" font-family="Arial" font-size="24" font-weight="bold">${project.name}</text>
...
async function main() {
  ...
  // Limpiar directorios
  await fs.remove(path.join(process.cwd(), "public", "projects"));
  await fs.remove(path.join(process.cwd(), "src", "data"));

  // Generar thumbnails
  await generateThumbnails();

  // Construir proyectos y generar datos
  const builtProjects = await generateProjectsData();

  console.log(
    `🎉 Build process completed! Built ${builtProjects.length} projects.`
  );
}
